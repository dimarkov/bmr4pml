...
def scale_by_lie(
  ...
  def init_fn(key, params):
    ...
    ε = jtu.tree_map(lambda key, t: n_init * jr.normal(key, shape=(S, *t.shape), dtype=m_dtype), keys, params)  # noise
  ...
  def update_fn(updates, state, params):
    ...
    V_k = jtu.tree_map(lambda s, g: s * g, state.A, updates)
    V = jtu.tree_map(lambda v: v.mean(0), V_k)
    U = jtu.tree_map(lambda v, e: jnp.mean(e * v, 0), V_k, state.ε)

    m_u = jtu.tree_map(lambda m, u: b2 * m + (1 - b2) * (u - t_tau), state.m_u, U)
    m_v = jtu.tree_map(lambda m, v: b1 * m + (1 - b1) * v, state.m_v, V)

    A = jtu.tree_map(lambda a, m: a * jnp.exp(- lr * a2 * m), state.A, m_u)
    updates = jtu.tree_map(
        lambda a, m1, m2: c_X * a * m2 * grpexp(lr * a1, m1) / c_y, state.A, m_u, m_v
      )
